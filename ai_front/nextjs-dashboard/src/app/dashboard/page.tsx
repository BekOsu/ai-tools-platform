...
export default function DashboardPage() {
  ...
  const stats = [
    { label: "AI Tools Used", value: "12", change: "+23%", icon: <FiActivity className="w-5 h-5" />, color: "bg-blue-500" },
    { label: "Hours Saved", value: "48", change: "+15%", icon: <FiClock className="w-5 h-5" />, color: "bg-green-500" },
    { label: "Favorite Tools", value: "5", change: "+2", icon: <FiHeart className="w-5 h-5" />, color: "bg-red-500" },
    { label: "Credits Left", value: `$${credits}`, change: "", icon: <FiTrendingUp className="w-5 h-5" />, color: "bg-purple-500" },
  ];
    ...
    <div className="space-y-8">
        ...
        {stats.map((stat, index) => (
          <div key={index} className="bg-white p-4 rounded-md border border-gray-300">
            <div className="flex items-start justify-between">
              ...
              <div className="p-2 rounded bg-gray-50 text-gray-600">{stat.icon}</div>
            </div>
          </div>
        ))}
      </div>
          ...
          <div className="p-4 space-y-3">
            {recentActivity.map((activity, index) => (
              <div key={index} className="flex items-center space-x-3 py-2">
                <div className={`w-2 h-2 rounded-full ${activity.status === 'completed' ? 'bg-green-500' : 'bg-red-500'}`}></div>
      ...
      <div className="bg-white rounded-md border border-gray-300">
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
          <button onClick={() => window.location.href = '/playground'} className="group flex items-center space-x-3 p-3 border border-gray-300 rounded-md hover:border-blue-500 hover:bg-blue-50 transition-all">
            <div className="w-8 h-8 bg-gray-100 rounded-md flex items-center justify-center group-hover:bg-blue-100">
          ...
          <button onClick={() => alert('Trading Analysis coming soon!')} className="group flex items-center space-x-3 p-3 border border-gray-300 rounded-md hover:border-gray-400 hover:bg-gray-50 transition-all">
            <div className="w-8 h-8 bg-gray-100 rounded-md flex items-center justify-center group-hover:bg-gray-200">
          ...
          <button onClick={() => alert('Image Processing coming soon!')} className="group flex items-center space-x-3 p-3 border border-gray-300 rounded-md hover:border-gray-400 hover:bg-gray-50 transition-all">
            <div className="w-8 h-8 bg-gray-100 rounded-md flex items-center justify-center group-hover:bg-gray-200">
      ...
      <div className="bg-white border border-blue-200 rounded-md p-4 text-center">
        ...
        <button onClick={() => window.location.href = '/'} className="bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700">
          Browse All Tools
        </button>
      </div>
    </div>
